...
export default function SignaturesPage() {
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">E-signeringar</h1>
        <p className="text-sm text-muted-foreground mt-1">Alla signeringsbegäran för organisationen</p>
      </div>

      {loading && <p className="text-sm text-muted-foreground">Laddar…</p>}

      {!loading && requests.length === 0 && (
        <div className="rounded-xl border border-dashed border py-16 text-center">
          <p className="text-sm text-muted-foreground mb-1">Inga signeringsbegäran ännu</p>
      ...
      {active.length > 0 && (
        <section className="mb-8">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Aktiva ({active.length})</h2>
      ...
      {done.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Avslutade ({done.length})</h2>
          <div className="space-y-2">
...
function RequestRow({ r, entityLink }: { r: Request; entityLink: string }) {
  ...
  return (
    <div className="rounded-xl border border bg-card p-4 flex items-center gap-4 hover:border transition-colors">
      <div className="flex-1 min-w-0">
        ...
        <Link href={entityLink} className="font-medium text-foreground text-sm hover:text-indigo-600 truncate block">
          {r.title}
        </Link>
      ...
      <div className="text-right flex-shrink-0">
        ...
        {["sent", "partially_signed"].includes(r.status) && (
          <p className="text-xs text-muted-foreground mt-0.5">Utgår {fmtDate(r.expiresAt)}</p>
        )}
      </div>
    </div>
  )
