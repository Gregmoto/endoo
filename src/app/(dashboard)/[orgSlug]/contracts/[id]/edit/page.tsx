...
export default function EditContractPage() {
    ...
    Promise.all([
      ...
      if (contract) {
        setForm({
          ...
          currency:         contract.currency ?? "SEK",
          paymentTermsDays: String(contract.paymentTermsDays ?? 30),
          autoSend:         contract.autoSend ?? false,
        ...
        setLines(
          (contract.lines ?? []).map((l: any) => ({
            ...
            unit:        l.unit,
            unitPriceKr: Number(l.unitPrice) / 100,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    router.push(`/${orgSlug}/contracts/${id}`)
  }

  const totalPeriod = lines.reduce((s, l) => {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6 flex items-center gap-3">
        <Link href={`/${orgSlug}/contracts/${id}`} className="text-sm text-muted-foreground hover:text-foreground">← Avtal</Link>
        <span className="text-gray-300">/</span>
        <h1 className="text-2xl font-bold text-foreground">Redigera avtal</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="p-0">
            ...
            <div className="px-4 py-3 border-t border text-right text-sm text-muted-foreground">
              Summa per period: <span className="font-bold text-foreground ml-2">
                {totalPeriod.toLocaleString("sv-SE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {form.currency}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/${orgSlug}/contracts/${id}`}>
