...
export default async function PlatformOrgDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="mb-6">
        <Link href="/platform/organizations" className="text-sm text-muted-foreground hover:text-muted-foreground">← Alla organisationer</Link>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Kunder" value={org._count.contacts} />
        ...
        <Card><CardContent className="p-4">
          ...
          <p className="text-lg font-bold text-foreground capitalize">{org.plan}</p>
        </CardContent></Card>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">{org._count.invoices} totalt</span>
        </CardHeader>
        <CardContent className="p-0">
          {org.invoices.length === 0 ? (
            <p className="px-6 py-8 text-sm text-muted-foreground text-center">Inga fakturor.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {org.invoices.map((inv) => (
                  <tr key={inv.id} className="border-t border-border/50">
                    <td className="px-6 py-3 font-mono text-xs text-foreground">{inv.invoiceNumber}</td>
                    <td className="px-6 py-3 text-foreground">{inv.contact?.name ?? "—"}</td>
