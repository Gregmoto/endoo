...
import { getBalanceSheet }    from "@/services/reports/balance-sheet"
...
function handleError(err: unknown): Response {
  ...
  console.error("[report:balance-sheet]", err)
...
export async function GET(req: NextRequest): Promise<Response> {
  try {
    ...
    const fiscalYearFromDate = searchParams.get("fromDate") ?? undefined

    if (!toDate) {
      return Response.json({ error: "toDate is required" }, { status: 400 })
    }
