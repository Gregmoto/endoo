...
import { getGeneralLedger }   from "@/services/reports/general-ledger"
...
function handleError(err: unknown): Response {
  ...
  console.error("[report:general-ledger]", err)
...
export async function GET(req: NextRequest): Promise<Response> {
  try {
    ...
    const accs           = searchParams.get("accountNumbers")
    ...
    const accountNumbers = accs ? accs.split(",").map((s) => s.trim()).filter(Boolean) : undefined
